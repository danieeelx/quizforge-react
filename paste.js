 ...
 */
export function parseQuestionBankDetailed(rawText) {
...
function splitFlattenedQuestionStarts(line) {
...
function extractAnswerKey(lines) {
    ...
    for (const line of lines) {
        ...
        if (pairs.length >= 2 || inKey) {
            ...
            if (inKey && pairs.length === 0 && matchQuestionStart(line))
                inKey = false;
        }
    }
    return map;
...
function parseAnswerTokens(value) {
    ...
    if (letters?.length)
        return uniqueTokens(letters);
    return [value.trim()].filter(Boolean);
}
function uniqueTokens(tokens) {
    return [...new Set(tokens.map((token) => token.trim().toUpperCase()).filter(Boolean))];
}
function tokenToOptionId(token, sourceOptions, optionObjects) {
    ...
    const matchingIndex = sourceOptions.findIndex((option) => normalizeText(option.text) === normalizeText(normalizedToken));
